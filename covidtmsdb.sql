...
/*---------Stored Prodeures---------*/
--
-- To insert a new admin record into the "tbladmin" table:
CREATE PROCEDURE insert_admin (IN AdminName VARCHAR(120), IN AdminuserName VARCHAR(20), IN MobileNumber INT(10), IN Email VARCHAR(120), IN Password VARCHAR(120))
BEGIN
    INSERT INTO tbladmin (AdminName, AdminuserName, MobileNumber, Email, Password) VALUES (AdminName, AdminuserName, MobileNumber, Email, Password);
END;

--
--To update an existing admin record in the "tbladmin" table:
CREATE PROCEDURE update_admin (IN ID INT(11), IN AdminName VARCHAR(120), IN AdminuserName VARCHAR(20), IN MobileNumber INT(10), IN Email VARCHAR(120), IN Password VARCHAR(120))
BEGIN
    UPDATE tbladmin SET AdminName = AdminName, AdminuserName = AdminuserName, MobileNumber = MobileNumber, Email = Email, Password = Password WHERE ID = ID;
END;

--
--To delete an existing admin record in the "tbladmin" table:
CREATE PROCEDURE delete_admin (IN ID INT(11))
BEGIN
    DELETE FROM tbladmin WHERE ID = ID;
END;

--
--To insert a new patient record into the "tblpatients" table:
CREATE PROCEDURE insert_patient (IN FullName VARCHAR(120), IN MobileNumber BIGINT(12), IN DateOfBirth DATE, IN GovtIssuedId VARCHAR(150), IN GovtIssuedIdNo VARCHAR(150), IN FullAddress VARCHAR(255), IN State VARCHAR(200))
BEGIN
    INSERT INTO tblpatients (FullName, MobileNumber, DateOfBirth, GovtIssuedId, GovtIssuedIdNo, FullAddress, State) VALUES (FullName, MobileNumber, DateOfBirth, GovtIssuedId, GovtIssuedIdNo, FullAddress, State);
END;

--
--To update an existing patient record in the "tblpatients" table:
CREATE PROCEDURE update_patient (IN id INT(11), IN FullName VARCHAR(120), IN MobileNumber BIGINT(12), IN DateOfBirth DATE, IN GovtIssuedId VARCHAR(150), IN GovtIssuedIdNo VARCHAR(150), IN FullAddress VARCHAR(255), IN State VARCHAR(200))
BEGIN
    UPDATE tblpatients SET FullName = FullName, MobileNumber = MobileNumber, DateOfBirth = DateOfBirth, GovtIssuedId = GovtIssuedId, GovtIssuedIdNo = GovtIssuedIdNo, FullAddress = FullAddress, State = State WHERE id = id;
END;

--
--To delete an existing patient record in the "tblpatients" table:
CREATE PROCEDURE delete_patient (IN id INT(11))
BEGIN
    DELETE FROM tblpatients WHERE id = id;
END;

/*-------Triggers-------*/
--To create a trigger that logs all changes made to the "tbladmin" table:
CREATE TRIGGER log_admin_changes
AFTER INSERT, UPDATE, DELETE ON tbladmin
FOR EACH ROW
BEGIN
    INSERT INTO admin_changes_log (change_type, admin_id, admin_name, change_date) VALUES (
        CASE
        ...
        COALESCE(NEW.AdminName, OLD.AdminName),
        NOW()
    );
END;

--To create a trigger that logs all changes made to the "tblpatients" table:
CREATE TRIGGER log_patient_changes
AFTER INSERT, UPDATE, DELETE ON tblpatients
FOR EACH ROW
BEGIN
    INSERT INTO patient_changes_log (change_type, patient_id, patient_name, change_date) VALUES (
        CASE
            WHEN NEW.id IS NULL THEN 'DELETE'
            WHEN OLD.id IS NULL THEN 'INSERT'
            ELSE 'UPDATE'
        END,
        COALESCE(NEW.id, OLD.id),
        COALESCE(NEW.FullName, OLD.FullName),
        NOW()
    );
END;

--To create a trigger that logs all changes made to the "tblphlebotomist" table:
CREATE TRIGGER log_phlebotomist_changes
AFTER INSERT, UPDATE, DELETE ON tblphlebotomist
FOR EACH ROW
BEGIN
    INSERT INTO phlebotomist_changes_log (change_type, phlebotomist_id, phlebotomist_name, change_date) VALUES (
        CASE
            WHEN NEW.id IS NULL THEN 'DELETE'
            WHEN OLD.id IS NULL THEN 'INSERT'
            ELSE 'UPDATE'
        END,
        COALESCE(NEW.id, OLD.id),
        COALESCE(NEW.phlebotomistName, OLD.phlebotomistName),
        NOW()
    );
END;
